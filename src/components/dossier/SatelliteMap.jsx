...
function MapController({
  ...
  function initDraw(mbMap) {
    ...
    if (!mbMap.getSource("obstacles-layer")) {
      mbMap.addSource("obstacles-layer", { type: "geojson", data: { type: "FeatureCollection", features: [] } });
      mbMap.addLayer({ id: "obstacles-fill", type: "fill",   source: "obstacles-layer", paint: { "fill-color": "#EF4444", "fill-opacity": 0.55 } });
      mbMap.addLayer({ id: "obstacles-line", type: "line",   source: "obstacles-layer", paint: { "line-color": "#EF4444", "line-width": 2 } });
      mbMap.addLayer({ id: "obstacles-label", type: "symbol", source: "obstacles-layer",
    ...
    mbMap.on("draw.create", async (e) => {
      ...
      const panId = await createPanFromCoordsRef.current(usedCoords, wasClipped ? draw.getAll().features.at(-1)?.id : e.features[0].id);

      // ── Analyse IA Vision — détection d'obstacles en arrière-plan ────────
      if (import.meta.env.VITE_ANTHROPIC_API_KEY) {
        try {
          // Capture la zone du polygone depuis le canvas Mapbox
          const ring = usedCoords[0];
          const lngs = ring.map(c => c[0]), lats = ring.map(c => c[1]);
          const panBbox = {
            minLng: Math.min(...lngs), maxLng: Math.max(...lngs),
            minLat: Math.min(...lats), maxLat: Math.max(...lats),
          };
          // Capture le canvas complet (la zone bbox sera utilisée pour les coords)
          const canvas = mbMap.getCanvas();
          const fullBase64 = canvas.toDataURL('image/png').replace(/^data:image\/png;base64,/, '');

          analyzeRoofObstacles(fullBase64, panBbox).then(obstacles => {
            if (!obstacles.length) return;
            // Stocke les obstacles sur le pan
            setPans(prev => prev.map(p => p.id === panId ? { ...p, detectedObstacles: obstacles } : p));
            // Affiche les zones rouges sur la carte
            const existingData = mbMap.getSource('obstacles-layer')?._data ?? { type: 'FeatureCollection', features: [] };
            const newFeatures = obstacles.map(o => ({
              type: 'Feature',
              geometry: { type: 'Polygon', coordinates: o.coords },
              properties: { label: o.label, type: o.type },
            }));
            mbMap.getSource('obstacles-layer')?.setData({
              type: 'FeatureCollection',
              features: [...(existingData.features ?? []), ...newFeatures],
            });
          }).catch(err => console.warn('[Vision]', err.message));
        } catch (err) {
          console.warn('[Vision capture]', err.message);
        }
      }
    });
